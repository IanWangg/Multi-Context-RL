...
class FourRooms(gym.Env):
    ...
    def __init__(self, goal=62, p=0, config=2, layout='3roomsh'):
        """
...
"""
    ...
    def render_state(self, wall=0.01, agent=1, reward=-0.01):
        occupancy = self.occupancy * wall
        current_grid = np.array(occupancy)
        current_grid[self.current_cell[0], self.current_cell[1]] = agent
        goal_cell = self.tocell[self.goal]
        current_grid[goal_cell[0], goal_cell[1]] = reward
        return current_grid

    def render_cube(self):
        cube = np.zeros([3, 13, 13])
        cube[0, :, :] = self.render_state(wall=1, agent=0, reward=0)
        cube[1, :, :] = self.render_state(wall=0, agent=2, reward=0)
        cube[2, :, :] = self.render_state(wall=0, agent=0, reward=3)
        return cube
    ...
    def step(self, action):
        ...
        if(self.config == 0):
            ...
            return self.render_cube(), reward, done, {}

class FourRoomsMatrix(FourRooms):
    def __init__(self, goal=62, p=0, layout='3roomsh'):
        FourRooms.__init__(self, goal=goal, p=p, config=1, layout=layout)
...
class FourRoomsNoTerm(FourRooms):
    ...
    """
    def __init__(self, p=0, dying=0, config=2, layout='3roomsh'):
